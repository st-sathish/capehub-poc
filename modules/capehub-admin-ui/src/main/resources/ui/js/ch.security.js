chSecurity = new (function() {
  ...
  this.loadRoles = function(callback) {
    ...
    $.ajax({
      ...
      async : false,
      error : function() {
        if (chUtils !== undefined) {
          chUtils.log("Could not retrieve roles list from " + ROLES_LIST_URL);
        }
      },
  ...
  this.renderAcl = function(aclData, roleData) {
	  ...
	  var createACLDocument = function(aclId) {
		  var out = '<?xml version="1.0" encoding="UTF-8" standalone="yes"?><acl xmlns="http://com.capestart.security">';
  ...
  this.createAcl = function(name) {
	  var emptyAcl = '<?xml version="1.0" encoding="UTF-8" standalone="yes"?><acl xmlns="http://com.capestart.security"></acl>';
	  $.ajax({
		  type: 'POST',
		  url: "/acl-manager/acl/",
		  data: {
  ...
  this._refresh = function() {
    if (!refreshing) {
      ...
      return $.Deferred(function (d) {
    	if(chSecurity.Configuration.state == 'groups') {
    	...
    	} else if(chSecurity.Configuration.state == 'acl') {
  ...
  this.init = function() {
	  ...
	  $('#security-' +  chSecurity.Configuration.state).attr('checked', true);
	  $('.state-filter-container').buttonset();
	  $('.state-filter-container input').click(function() {
		  chSecurity.Configuration.state = $(this).val();
		  self.refresh();
	  });
