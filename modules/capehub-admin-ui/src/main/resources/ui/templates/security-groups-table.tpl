<table id="groupsTable" class="ui-widget" cellspacing="0" width="100%">
  ...
  <tbody>
  	<% var self = this; %>
    <% _.each(chUtils.ensureArray(this.groups.group), function(group) { %>
      <tr valign="top">
						...
						<ul class="oc-ui-form-list">
							...
							<li>
							    <select id="rolesSelect" name="top5" size="5" multiple>
							      <% _.each(chUtils.ensureArray(group.roles.role), function(role) { %>
							...
							<li>
							    ...
							    <select id="usersSelect" name="top5" size="5" multiple>
							      <% _.each(chUtils.ensureArray(group.members.member), function(member) { %>
