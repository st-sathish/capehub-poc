...
if [ ! -z "$CAPEHUB_LOGDIR" ]; then
  LOGDIR="$CAPEHUB_LOGDIR"
else
...
CAPEHUB_LOGGING_OPTS="-Dcapehub.logdir=$LOGDIR"
...
LOG_OPTS="$PAX_LOGGING_OPTS $CAPEHUB_LOGGING_OPTS $ECLIPSELINK_LOGGING_OPTS $UTIL_LOGGING_OPTS"
...
# Make sure capehub bundles are reloaded
if [ -d "$FELIX_CACHE" ]; then
  cd "$FELIX_CACHE"
  echo "Removing cached capehub bundles from $FELIX_CACHE"
